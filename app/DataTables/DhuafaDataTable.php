...
class DhuafaDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tanggal_lahir', function ($dhuafa) {
                return date('d/m/Y', strtotime($dhuafa->tanggal_lahir));
            })
            ->addColumn('action', function($dhuafa){
                return '
                    <a href="' . route('dhuafa.edit', $dhuafa->id) . '" class="btn btn-sm btn-warning text-white px-3"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                    
                    <form action="' . route('dhuafa.destroy', $dhuafa->id) . '" method="POST" class="d-inline">
                    ' . csrf_field() . '
                    ' . method_field('delete') . '
     ...
     */
    public function query(Dhuafa $model): QueryBuilder
    {
