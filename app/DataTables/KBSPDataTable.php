...
class KBSPDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tanggal_lahir', function ($kbsp) {
                return date('d/m/Y', strtotime($kbsp->tanggal_lahir));
            })
            ->addColumn('action', function($kbsp){
                return '
                    <a href="' . route('kbsp.edit', $kbsp->id) . '" class="btn btn-sm btn-warning text-white px-3"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                    
                    <form action="' . route('kbsp.destroy', $kbsp->id) . '" method="POST" class="d-inline">
                    ' . csrf_field() . '
                    ' . method_field('delete') . '
     ...
     */
    public function query(KBSP $model): QueryBuilder
    {
