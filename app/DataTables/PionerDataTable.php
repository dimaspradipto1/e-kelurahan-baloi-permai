...
class PionerDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tanggal_lahir', function($pioner){
                return date('d/m/Y', strtotime($pioner->tanggal_lahir));
            })
            ->addColumn('action', function($pioner){
                return '
                <a href="' . route('pioner.edit', $pioner->id) . '" class="btn btn-sm btn-warning text-white px-3"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                    
                <form action="' . route('pioner.destroy', $pioner->id) . '" method="POST" class="d-inline">
                ' . csrf_field() . '
                ' . method_field('delete') . '
     ...
     */
    public function query(Pioner $model): QueryBuilder
    {
