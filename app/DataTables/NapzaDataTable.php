...
class NapzaDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tanggal_lahir', function ($napza) {
                return date('d/m/Y', strtotime($napza->tanggal_lahir));
            })
            ->addColumn('action', function($napza){
                return '
                    <a href="' . route('napza.edit', $napza->id) . '" class="btn btn-sm btn-warning text-white px-3"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                    
                    <form action="' . route('napza.destroy', $napza->id) . '" method="POST" class="d-inline">
                    ' . csrf_field() . '
                    ' . method_field('delete') . '
     ...
     */
    public function query(Napza $model): QueryBuilder
    {
