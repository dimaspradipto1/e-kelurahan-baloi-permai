...
class BWBLPDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tanggal_lahir', function ($bwblp) {
                return date('d/m/Y', strtotime($bwblp->tanggal_lahir));
            })
            ->addColumn('action', function($bwblp){
                return '
                    <a href="' . route('bwblp.edit', $bwblp->id) . '" class="btn btn-sm btn-warning text-white px-3"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                    
                    <form action="' . route('bwblp.destroy', $bwblp->id) . '" method="POST" class="d-inline">
                    ' . csrf_field() . '
                    ' . method_field('delete') . '
     ...
     */
    public function query(BWBLP $model): QueryBuilder
    {
