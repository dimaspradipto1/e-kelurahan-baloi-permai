...
class PMBSDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tanggal_lahir', function ($pmbs) {
                return date('d/m/Y', strtotime($pmbs->tanggal_lahir));
            })
            ->addColumn('action', function($pmbs){
                return '
                    <a href="' . route('pmbs.edit', $pmbs->id) . '" class="btn btn-sm btn-warning text-white px-3"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                    
                    <form action="' . route('pmbs.destroy', $pmbs->id) . '" method="POST" class="d-inline">
                    ' . csrf_field() . '
                    ' . method_field('delete') . '
     ...
     */
    public function query(PMBS $model): QueryBuilder
    {
