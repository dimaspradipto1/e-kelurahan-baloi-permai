...
class ODHADataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tanggal_lahir', function ($odha) {
                return date('d/m/Y', strtotime($odha->tanggal_lahir));
            })
            ->addColumn('action', function($odha){
                return '
                    <a href="' . route('odha.edit', $odha->id) . '" class="btn btn-sm btn-warning text-white px-3"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                    
                    <form action="' . route('odha.destroy', $odha->id) . '" method="POST" class="d-inline">
                    ' . csrf_field() . '
                    ' . method_field('delete') . '
     ...
     */
    public function query(ODHA $model): QueryBuilder
    {
