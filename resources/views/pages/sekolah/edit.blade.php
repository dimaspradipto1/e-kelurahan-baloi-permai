                ...
                <form action="{{ route('sekolah.update', $sekolah->id) }}" method="POST">
                  ...
                  <div class="card-body">
                    <div class="form-group">
                      ...
                      <input type="text" name="nama" value="{{ old('nama') ?? $sekolah->nama }}" class="form-control @error('nama') is-invalid @enderror" id="nama" placeholder="masukkan nama sekolah" >
                      @error('nama')
                    ...
                    <div class="form-group">
                      ...
                      <textarea placeholder="masukkan alamat sekolah..." name="alamat" class="form-control @error('alamat') is-invalid @enderror" cols="30"
                    ...
                    <div class="form-group">
                     ...
                     <select name="rt" class="form-control select2 @error('rt') is-invalid @enderror">
                      ...
                      @foreach ($rts as $item)
                      <option value="{{ $item->rt }}" {{ $item->rt == $sekolah->rt ? 'selected' : '' }}>{{ $item->rt }}</option>
                      @endforeach
                     </select>
                      @error('rt')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group">
                      ...
                      <select name="rw" class="form-control @error('rw') is-invalid @enderror select2">
                        ...
                        @foreach($rws as $item)
                        <option value="{{ $item->rw }}" {{ $item->rw == $sekolah->rw ? 'selected' : '' }}>{{ $item->rw }}</option>
                        @endforeach
                      </select>
                      @error('rw')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group">
                     ...
                     <select name="status"  id="status" class="form-control @error ('status') is-invalid @enderror">
                      <option selected> {{ $sekolah->status }} </option>
