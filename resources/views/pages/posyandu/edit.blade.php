    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>Form Edit Data Posyandu</h1>
            </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item active">Form Edit Data Posyandu</li>
              </ol>
            </div>
          </div>
        </div><!-- /.container-fluid -->
            ...
            <div class="col-md-8">
              ...
              <div class="card card-success">
                <div class="card-header">
                  <h3 class="card-title text-white">Edit Data Posyandu</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form action="{{ route('posyandu.update', $posyandu->id) }}" method="POST">
                  ...
                  <div class="card-body">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" name="nama" value="{{ old('nama') ?? $posyandu->nama }}" class="form-control @error('nama') is-invalid @enderror" id="nama" placeholder="masukkan nama lembaga" >
                      @error('nama')
                    ...
                    <div class="form-group">
                      ...
                      rows="3">{{ old('alamat') ?? $posyandu->alamat }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                     ...
                     <select name="rt" class="form-control select2 @error('rt') is-invalid @enderror">
                      ...
                      @foreach ($rts as $item)
                      <option value="{{ $item->rt }}" {{ $item->rt == $posyandu->rt ? 'selected' : '' }}>{{ $item->rt }}</option>
                      @endforeach
                     </select>
                      @error('rt')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group">
                      ...
                      <select name="rw" class="form-control @error('rw') is-invalid @enderror select2">
                        ...
                        @foreach($rws as $item)
                        <option value="{{ $item->rw }}" {{ $item->rw == $posyandu->rw ? 'selected' : ''}}>{{ $item->rw }}</option>
                        @endforeach
                      </select>
                      @error('rw')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group">
                      ...
                      <input type="number" name="no_hp" value="{{ old('no_hp') ?? $posyandu->no_hp }}" class="form-control @error('no_hp') is-invalid @enderror" id="no_hp" placeholder="masukkan no hp">
                      @error('no_hp')
                        <div class="invalid-feedback">{{ $message }}</div>
                      @enderror  
                    </div>
